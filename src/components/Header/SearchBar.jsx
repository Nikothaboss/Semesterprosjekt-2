...
const DataItem = ({image_url, title, price}) => {
    return(
    <DataItemStyled justifyContent="space-between" alignItems="center" flexDir="row">
        ...
        {filteredData.length !== 0 && inputValue.trim() !== "" && (
            <Flex className="dataResults" flexDir="column">
                {filteredData.map((item =>{
                    return (
                        <DataItem image_url={item.image_url} title={item.title} price={item.price} /> 
                    )
                }))}
            </Flex>
        )}
